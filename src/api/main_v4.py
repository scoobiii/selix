...
# ============================================================
# CRIAÇÃO AUTOMÁTICA DE TABELAS (para testes e sandbox)
# ============================================================
...
def criar_tabelas():
    ...
    cur = conn.execute("SELECT COUNT(*) FROM commodities")
    if cur.fetchone()[0] == 0:
        conn.execute("INSERT INTO commodities (nome, preco_usd, fonte) VALUES ('Brent', 95.19, 'yahoo_finance')")

    cur = conn.execute("SELECT COUNT(*) FROM selic_historico")
    if cur.fetchone()[0] == 0:
        conn.execute("INSERT INTO selic_historico (tipo, valor, fonte) VALUES ('efetiva', 14.25, 'bcb')")
        conn.execute("INSERT INTO selic_historico (tipo, valor, fonte) VALUES ('selic_ideal', 9.25, 'selix')")

    cur = conn.execute("SELECT COUNT(*) FROM api_keys")
    if cur.fetchone()[0] == 0:
        import hashlib
        key_hash = hashlib.sha256("selix_salt_2026test_api_key_123".encode()).hexdigest()
        conn.execute(
            "INSERT INTO api_keys (key_hash, client_name, plan, is_active) VALUES (?, ?, ?, ?)",
            (key_hash, "test_client", "pro", 1)
        )
