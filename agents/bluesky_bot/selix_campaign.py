...
# Versão 3.2.0-STABLE (link direto Cloud Run + lock + rate-limit control)
# Gera calendário + posts diários para todos os stakeholders
# Uso: python selix_campaign.py [--dry-run] [--day N] [--post] [--thread] [--link URL]
# Sem --link, usa o link padrão do SELIX Cloud Run

import os, sys, json, time, sqlite3, requests, argparse, fcntl
...
# Link padrão (direto, sem redirecionamento, sem propaganda)
DEFAULT_LINK = "https://selix-555922434592.us-west2.run.app/"
MAX_CHARS = 295          # limite seguro (Bluesky permite 300)

# Lock para evitar múltiplas execuções simultâneas (previne rate limit)
LOCK_FILE = "/tmp/selix_campaign.lock"
...
def gerar_calendario(d, link_agent=None):
    """
    Retorna dict {dia: [post1, post2, post3]}
    Cada post é um dict {texto, segmento, horario}
    link_agent: URL personalizada – se None, usa DEFAULT_LINK
    """
    brent = d['brent']
    sv    = d['sv']
    em    = d['e_mix']
    bm    = d['b_mix']
    sent  = d['sent']
    R     = link_agent if link_agent else DEFAULT_LINK   # link direto Cloud Run
    ...
    # ── CALENDÁRIO: rotação de segmentos por dia (mesma da versão anterior) ──
    rotacao = [
    ...
    for dia, (seg1, seg2, seg3) in enumerate(rotacao, start=1):
        ...
        for i, seg in enumerate(segs):
            ...
            # Garantir que o texto não ultrapasse o limite
            texto = trunc(texto, MAX_CHARS)
            posts.append({
                "segmento": seg,
                "horario":  horarios[i],
                "texto":    texto,
...
def trunc(text, limit=MAX_CHARS):
    """Corta o texto no limite seguro, adicionando '…' se necessário."""
...
def publicar_posts(client, posts_dia):
    ...
    for p in posts_dia:
        texto_final = trunc(p['texto'], MAX_CHARS)
        resultado = client.send_post(texto_final)
        print(f"  ✅ [{p['segmento']:20}] {p['horario']} → {resultado.uri}")
        time.sleep(10)   # aumento para evitar rate limit
...
def publicar_thread_dia(client, posts_dia):
    ...
    if not posts_dia:
        return
    primeiro = trunc(posts_dia[0]['texto'], MAX_CHARS)
    root = client.send_post(primeiro)
    ...
    for p in posts_dia[1:]:
        ...
        prev = client.send_post(text=texto_final, reply_to=reply_ref)
...
def main():
    ...
    try:
        ...
        print("❌ Outra instância do campaign já está rodando. Abortando para evitar rate limit.")
        sys.exit(1)
    ...
    parser.add_argument("--link", type=str, default=None,
                        help="URL personalizada (padrão: link direto do Cloud Run)")
    ...
    cal = gerar_calendario(d, link_agent=args.link)
    ...
    if args.day:
        ...
        print(f"   Link padrão:        {DEFAULT_LINK}")
        print("   Usar outro link:    python selix_campaign.py --day 1 --post --link \"https://meu.link\"")
